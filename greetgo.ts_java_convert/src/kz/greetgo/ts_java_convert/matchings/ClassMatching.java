package kz.greetgo.ts_java_convert.matchings;
...
public class ClassMatching {
  ...
  private ClassMatching(String lineClassName, String extend, List<String> impls) {
  ...
  public static ClassMatching match(String line) {
    {
      Matcher matcher = CLASS_DEFINITION1.matcher(line);
      if (matcher.matches()) {
        String lineClassName = matcher.group(1);
        return new ClassMatching(lineClassName, null, Collections.emptyList());
      }
    }

    {
      ...
      if (matcher.matches()) {
        ...
        return new ClassMatching(lineClassName, extend, Collections.emptyList());
      }
    }

    {
      ...
      if (matcher.matches()) {
        ...
        return new ClassMatching(lineClassName,
          null,
